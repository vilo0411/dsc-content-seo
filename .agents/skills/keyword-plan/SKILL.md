...
4. **Kiểm tra SERP signal:** chạy `python scripts/serp_opportunities.py` (0 API call) rồi đọc section "1. Chưa có bài" trong `knowledge/3-pipeline/serp-opportunities.md`. Câu PAA / related search xuất hiện ở ≥ 2 keyword đã research và chưa có bài → ưu tiên ngang GSC signal (Google đã xác nhận nhu cầu). Related search khác intent với bài gốc (VD "cách mua cổ phiếu Vingroup") là ứng viên bài riêng, không phải secondary keyword.
5. Phân tích ngữ cảnh, độ khó, và tính cấp thiết để lọc ra [N] từ khóa phù hợp nhất.
6. Ghi nhận các từ khóa này vào `knowledge/4-content/sprint-backlog.md` với trạng thái `Planned`.
